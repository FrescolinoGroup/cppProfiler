...
namespace detail {
    union rdtsc_struct {
        double cycle() const { return _tcs; }
    ...
    inline double operator-(rdtsc_struct const& end,
                            rdtsc_struct const& begin) {
...
public:
    ...
    void start() {
        ...
        asm volatile("rdtsc"
                     : "=a"((cyc_start_).split.lo),
                       "=d"((cyc_start_).split.hi));  // RDTSC
    }
    void stop() {
        asm volatile("rdtsc"
                     : "=a"((cyc_stop_).split.lo),
                       "=d"((cyc_stop_).split.hi));  // RDTSC
        asm volatile("cpuid"
                     :
                     : "a"(0)
                     : "bx", "cx", "dx");  // CPUID exec-barrier
    ...
    //------------------------- const methods ------------------------------
    double cycles() const { return cyc_stop_ - cyc_start_; }
    double sec() const {
        using sec_dur = std::chrono::duration<double, std::ratio<1, 1>>;
        auto res =
            std::chrono::duration_cast<sec_dur>(chrono_stop_ - chrono_start_);
        return res.count();
    }
    double usec() const {
        using usec_dur = std::chrono::duration<double, std::ratio<1, 1000000>>;
        auto res =
            std::chrono::duration_cast<usec_dur>(chrono_stop_ - chrono_start_);
        return res.count();
    }
    double nsec() const {
        using usec_dur =
...
inline std::ostream& operator<<(std::ostream& os, rdtsc_timer const& arg) {
    os << "cycles measured with rdtsc:        " << arg.cycles() << " cycles"
       << std::endl;
