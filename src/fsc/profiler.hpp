...
public:
    ...
    inline void stop(std::string const& name) { process_stop(name, get_cyc()); }
    void print_tree() const {
        ...
        std::cout << std::setw(8 + 14) << "┬ (% of parent) name"
                  << std::string(80 - 53, ' ') << std::right << std::setw(10)
                  << msm_mod_str << std::setw(10 + 1) << "spread"
                  << std::setw(10) << "run_cnt" << std::endl;
        ...
        for(auto const& p : sentinel_.children) {
            ...
            print_tree_helper(p, i == 0, marker, self_cost, p->cost(mod_));
        }
        print_missing_helper(child_cost, self_cost, marker);
    ...
    void set_mod(std::string const& smod) {
        ...
        if(smod == "cycle" or smod == "cyc" or smod == "")
    ...
    void register_help() const {
        std::cout << "To change to register mode, paste the following lines "
    ...
    void save(std::string file) const {
        ...
        std::string smod = mod_ == detail::msm_mod::cycle ? "cycle" : "nsec";
...
private:
    ...
    void print_tree_helper(const detail::node_struct* parent,
        ...
        if(parent->children.size()) {
            print_missing_helper(child_cost, self_time, marker);
        }
        marker.pop_back();
    }
...
#define MIB_INFO() \
    { std::cout << "you are already in register mode" << std::endl; }
...
#define MIB_NEXT(tag1, tag2) \
    { MIB_STOP(tag1) MIB_START(tag2) }
