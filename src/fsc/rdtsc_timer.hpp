...
namespace util {
    ...
    public:
        ...
        void start() {
            ...
            asm volatile("rdtsc"
                         : "=a"((cyc_start_).lo),
                           "=d"((cyc_start_).hi));  // RDTSC
        }
        void stop() {
            asm volatile("rdtsc"
                         : "=a"((cyc_stop_).lo),
                           "=d"((cyc_stop_).hi));  // RDTSC
            asm volatile("cpuid"
                         :
                         : "a"(0)
                         : "bx", "cx", "dx");  // CPUID exec-barrier
        ...
        double sec() const {
            ...
            auto res = std::chrono::duration_cast<sec_dur>(chrono_stop_ -
                                                           chrono_start_);
            return res.count();
        }
        double usec() const {
            ...
            auto res = std::chrono::duration_cast<usec_dur>(chrono_stop_ -
                                                            chrono_start_);
            return res.count();
        }
        double nsec() const {
